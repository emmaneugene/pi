/**
 * Pi extension: `/context` command
 *
 * Displays an overlay visualising how the current context window is being
 * consumed, broken down by category:
 *
 *   - **System Prompt** – the static system prompt injected at the start of
 *     every request.
 *   - **System Tools** – token cost of the active tool definitions sent to
 *     the model.
 *   - **Tool Call** – combined cost of tool-use invocations and their results
 *     accumulated in the conversation branch.
 *   - **Messages** – user and assistant text turns.
 *   - **Other** – any remaining tokens reported by the API that don't map to
 *     a specific category above (e.g. cache overhead).
 *   - **Available** – unused tokens remaining in the context window.
 *
 * Token counts per category are estimated locally from raw text length
 * (1 token ≈ 4 chars) and then scaled proportionally so the sum matches the
 * exact total reported by the model API.
 */
import {
  type ExtensionAPI,
  type SessionManager,
