 ...
 *
 * Source reference:
 * - Original upstream implementation: https://github.com/mitsuhiko/agent-stuff/blob/main/pi-extensions/files.ts
 */
...
const buildFileEntries = async (
  ...
  const upsertFile = (
    ...
    fileMap.set(data.canonicalPath, {
      ...
      operations: new Set(data.operations ?? []),
...
> => {
  const actions: SelectItem[] = [
    ...(options.canDiff ? [{ value: "diff", label: "Diff in IDE" }] : []),
...
const showFileSelector = async (
  ...
  const selection = await ctx.ui.custom<string | null>(
    (tui, theme, keybindings, done) => {
      const renderOperationBadges = (file: FileEntry): string => {
        const ops: string[] = [];
        ...
        return `${theme.fg("dim", "[")}${ops.join("")}${theme.fg("dim", "] ")}`;
      };
      ...
      const items: SelectItem[] = files.map((file) => {
        ...
        return {
          ...
          label: `${renderOperationBadges(file)}${file.displayPath}${directoryLabel}${statusSuffix}`,
        };
      });
      const container = new Container();
      container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
