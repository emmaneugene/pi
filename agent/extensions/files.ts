...
export default function (pi: ExtensionAPI) {
  pi.registerCommand("files", {
    ...
    handler: async (_args, ctx) => {
      ...
      const openWithIDE = async (path: string) => {
        if (process.platform === "win32") {
          ...
          const commandLine = `${IDE_COMMAND} ${quoteCmdArg(path)}`;
          return pi.exec("cmd", ["/d", "/s", "/c", commandLine], {
            cwd: ctx.cwd,
          });
        }
        return pi.exec(IDE_COMMAND, [path], { cwd: ctx.cwd });
      };

      const openSelected = async (file: FileEntry): Promise<void> => {
        try {
          const openResult = await openWithIDE(file.path);
