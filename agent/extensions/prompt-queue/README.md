...
Pi loads the extension automatically from `~/.pi/agent/extensions/prompt-queue/`. To load only this extension for one session:

```bash
pi --no-extensions -e ~/.pi/agent/extensions/prompt-queue/index.ts
```
