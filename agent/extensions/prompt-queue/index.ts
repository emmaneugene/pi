...
type ManagerOutcome = Extract<
  ManagerResult,
  { kind: "close" | "resume" | "send-now" }
>;
...
export class PromptQueueRuntime {
  ...
  deliverNextWhenIdle(): void {
    ...
    if (
      decideSettledDelivery(this.gate, this.snapshot(), undefined) !==
      "deliver-next"
    )
      return;
  ...
  onSettled(): void {
    ...
    if (this.sendOnSettle !== undefined) {
      ...
      const decision = decideSettledDelivery(
        this.gate,
        this.snapshot(),
        finalOutcome,
      );
  ...
  private holdQueue(reason: HoldReason): void {
    ...
    const prefix =
      reason === "error"
        ? "Prompt queue paused after an agent error"
        : "Prompt queue paused";
    this.ctx?.ui.notify(
    ...
    for (;;) {
      const result = await ctx.ui.custom<ManagerResult>(
        (tui, theme, _keybindings, done) =>
          new ManagerWindow(state, tui, theme, done),
      );
      if (
        result.kind === "close" ||
        result.kind === "resume" ||
        result.kind === "send-now"
      )
        return result;
  ...
  async openManager(): Promise<void> {
    ...
    try {
      outcome = await this.runManager(
        ctx,
        new ManagerWindowState(this.queue, this.history),
      );
    } finally {
  ...
  openManagerSafely(): void {
    this.openManager().catch((error: unknown) => {
      this.ctx?.ui.notify(
        `Prompt queue manager failed: ${String(error)}`,
        "error",
      );
    });
  }
...
export default function promptQueue(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("queue", {
    description:
      "Open the prompt queue and history manager; `resume` continues delivery",
    getArgumentCompletions: (prefix) =>
      "resume".startsWith(prefix)
    ...
    handler: async (args, ctx) => {
      ...
      if (ctx.mode !== "tui") {
        ctx.ui.notify(
          "The prompt queue manager needs interactive mode.",
          "warning",
        );
        return;
      }
