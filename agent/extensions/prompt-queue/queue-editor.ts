...
import {
  decideForcedEnqueue,
  decideSubmit,
  shouldOpenManagerOnUp,
} from "./submit-policy.ts";
...
export class QueuePromptEditor extends CustomEditor {
  ...
  private interceptKey(data: string): boolean {
    ...
    if (this.bindings.matches(data, "tui.input.tab"))
      return this.submitForced("steer");
    if (this.bindings.matches(data, "app.message.followUp"))
      return this.submitForced("queue");
    return false;
  }
