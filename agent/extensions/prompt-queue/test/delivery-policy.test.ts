...
describe("decideTurnEndDelivery", () => {
  it("delivers a steer item on a normal turn boundary", () => {
    expect(decideTurnEndDelivery(open, both, "completed")).toBe(
      "deliver-steer",
    );
  ...
  it("pauses while the manager window is open", () => {
    expect(
      decideTurnEndDelivery(
        { windowOpen: true, holdReason: undefined },
        both,
        "completed",
      ),
    ).toBe("none");
  ...
  it("pauses while delivery has a hold reason", () => {
    expect(
      decideTurnEndDelivery(
        { windowOpen: false, holdReason: "abort" },
        both,
        "completed",
      ),
    ).toBe("none");
    expect(
      decideTurnEndDelivery(
        { windowOpen: false, holdReason: "error" },
        both,
        "completed",
      ),
    ).toBe("none");
...
describe("decideSettledDelivery", () => {
  it("delivers the next pending item after a successful settle", () => {
    ...
    expect(
      decideSettledDelivery(open, { hasSteer: false, hasAny: true }, undefined),
    ).toBe("deliver-next");
  });
  ...
  it("does nothing with an empty queue", () => {
    expect(
      decideSettledDelivery(open, { hasSteer: false, hasAny: false }, "error"),
    ).toBe("none");
  });
