...
describe("decideTurnEndDelivery", () => {
  ...
  it("does nothing without pending steer items", () => {
    expect(
      decideTurnEndDelivery(open, { hasSteer: false, hasAny: true }, "stop"),
    ).toBe("none");
  });

  it("pauses while the manager window is open", () => {
    expect(
      decideTurnEndDelivery({ windowOpen: true, held: false }, both, "stop"),
    ).toBe("none");
  });

  it("pauses while delivery is held after an abort", () => {
    expect(
      decideTurnEndDelivery({ windowOpen: false, held: true }, both, "stop"),
    ).toBe("none");
  });
...
describe("decideIdleDelivery", () => {
  it("delivers the next pending item when idle", () => {
    ...
    expect(decideIdleDelivery(open, { hasSteer: false, hasAny: true })).toBe(
      "deliver-next",
    );
  });

  it("does nothing with an empty queue", () => {
    expect(decideIdleDelivery(open, { hasSteer: false, hasAny: false })).toBe(
      "none",
    );
  });

  it("pauses while the manager window is open or delivery is held", () => {
    expect(decideIdleDelivery({ windowOpen: true, held: false }, both)).toBe(
      "none",
    );
    expect(decideIdleDelivery({ windowOpen: false, held: true }, both)).toBe(
      "none",
    );
  });
});
