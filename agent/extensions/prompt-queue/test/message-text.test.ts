...
describe("assistantStopReason", () => {
  it("returns the stop reason of an assistant message", () => {
    expect(
      assistantStopReason({ role: "assistant", stopReason: "aborted" }),
    ).toBe("aborted");
    expect(assistantStopReason({ role: "assistant", stopReason: "stop" })).toBe(
      "stop",
    );
  });
  ...
  it("returns undefined for non-assistant messages and malformed values", () => {
    expect(
      assistantStopReason({ role: "user", stopReason: "stop" }),
    ).toBeUndefined();
    expect(assistantStopReason({ role: "assistant" })).toBeUndefined();
    expect(
      assistantStopReason({ role: "assistant", stopReason: 3 }),
    ).toBeUndefined();
...
describe("userMessageText", () => {
  ...
  it("returns undefined for non-user messages and malformed content", () => {
    expect(
      userMessageText({ role: "assistant", content: "x" }),
    ).toBeUndefined();
