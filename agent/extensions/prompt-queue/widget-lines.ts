...
function itemLine(
  ...
  const label =
    item.mode === "steer"
      ? palette.warning("will steer")
      : palette.accent("queued");
...
function statusLine(gate: DeliveryGate, palette: WidgetPalette): string {
  if (gate.windowOpen) {
    return (
      palette.warning("prompt queue paused") +
      palette.dim(" — press ↑ to manage")
    );
  }
  if (gate.holdReason !== undefined) {
    const reason = gate.holdReason === "error" ? "agent error" : "abort";
    return (
      palette.warning(`prompt queue paused after ${reason}`) +
      palette.dim(" — press ↑ to manage")
    );
  }
...
export function widgetLines(
  ...
  if (items.length === 0 && gate.holdReason === undefined && !gate.windowOpen)
    return [];
  const shown = items.slice(0, MAX_WIDGET_ITEMS);
  const lines = shown.map((item, index) => itemLine(item, index + 1, palette));
  if (items.length > shown.length) {
    lines.push(
      palette.dim(`… ${String(items.length - shown.length)} more queued`),
    );
  }
