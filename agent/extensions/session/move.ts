 ...
 * running session, so switching into a different-cwd session would leave
 * the process cwd and tool execution dir pointing at the old directory.
 *
 * After the move, resume in the target dir with:  cd <target> && pi -c
 */
...
// Parse a session header and require a usable `id`. Every downstream path
// (subagent relocation, the resume command) needs `id` to be a real string;
// validating here, before any write, keeps a malformed header from leaving a
...
function parseSessionHeader(
  ...
  if (typeof header?.id !== "string" || !header.id) {
    throw new Error("session header is missing a valid id");
  }
  return header;
}
 ...
 * per-parent folder `<sessionDir>/subagents/<parentSessionId>/`, so
 * ownership is structural: this function does no header filtering.
 *
 * Each child still embeds an absolute `cwd` inherited from the parent. This
 * function rewrites that `cwd` while it copies each child into the
 * destination folder, then removes the original folder entirely.
 */
