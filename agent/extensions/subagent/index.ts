...
export default function (pi: ExtensionAPI) {
  ...
  function getBaselineTools(): string[] {
    ...
    baselineTools = pi
      .getActiveTools()
      .filter((name) => name !== SUBAGENT_TOOL_NAME);
  ...
  function applyToolState(ctx: ExtensionContext): void {
    ...
    const nextTools = subagentModeEnabled
      ? [...new Set([...base, SUBAGENT_TOOL_NAME])]
      : base;
  ...
  function setSubagentMode(enabled: boolean, ctx: ExtensionContext): void {
    ...
    ctx.ui.notify(
      enabled
        ? "Subagent mode enabled. Delegation tool available."
      ...
      if (!subagentModeEnabled) {
        return {
          content: [
            {
              type: "text",
              text: "Subagent mode is disabled. Enable it with /subagents first.",
            },
          ],
          details: {
            mode: "single",
    ...
    const modeEntry = ctx.sessionManager
      ...
      .filter(
        (e: { type: string; customType?: string }) =>
          e.type === "custom" && e.customType === SUBAGENT_MODE_ENTRY,
      )
