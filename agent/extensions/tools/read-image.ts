...
const modelSupportsImages = (model: Model<any> | undefined): boolean =>
...
export default function (pi: ExtensionAPI) {
  const syncReadImageTool = (model: Model<any> | undefined) => {
    const activeTools = pi.getActiveTools();
    const isActive = activeTools.includes(TOOL_NAME);
    const shouldBeActive = !modelSupportsImages(model);
    if (shouldBeActive === isActive) return;

    pi.setActiveTools(
      shouldBeActive
        ? [...activeTools, TOOL_NAME]
        : activeTools.filter((name) => name !== TOOL_NAME),
    );
  };
