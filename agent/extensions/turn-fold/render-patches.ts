...
export function formatSettledSummary(summary: FoldSummary): string {
  ...
  if (summary.failedTools > 0)
    parts.push(countLabel(summary.failedTools, "failure"));
...
function styledSummary(
  ...
  const styled = theme
    ? theme.bold(
        theme.fg(
          summary.aborted || summary.failedTools > 0 ? "warning" : "muted",
          truncated,
        ),
      )
    : truncated;
...
function settledFinal(
  ...
  const visible =
    original.length === 0 && summary.aborted
      ? interruptionFallback(theme, width)
      : original;
...
function timestampAfterContent(
  ...
  if (timestamp === undefined || width <= 0 || original.length === 0)
    return original;
...
function installUserSpacingPatches(): RestoreRenderPatches {
  ...
  const patchedAddChild = function (this: Container, component: Child): void {
    ...
    if (previous instanceof Spacer && isUserRow(component))
      suppressedSpacers.add(previous);
    originalAddChild.call(this, component);
  };
  const patchedSpacerRender = function (this: Spacer, width: number): string[] {
    return suppressedSpacers.has(this)
      ? []
...
function installUserTimestampPatches(
  ...
  const patchedUserRender = function (
  ...
  return () => {
    restoreUserSpacing();
    if (userPrototype.render === patchedUserRender)
      userPrototype.render = originalUserRender;
    if (skillPrototype.render === patchedSkillRender)
      skillPrototype.render = originalSkillRender;
  };
}
...
function renderFoldView(
  ...
  if (display === "streaming-summary")
    return renderStreamingSummary(summary, width, theme);
  if (display === "settled-summary")
    return renderSettledSummary(summary, width, theme);
...
export function installRenderPatches(
  ...
  const patchedToolMarkExecutionStarted = function (
  ...
  const patchedToolRender = function (
    this: ToolExecutionComponent,
    width: number,
  ): string[] {
