...
import {
  ...
  type RestoreRenderPatches,
} from "../render-patches.ts";
import { TurnFoldState } from "../turn-state.ts";

type AssistantMessage = NonNullable<
...
it("renders local user and completion times in transcript order", () => {
  ...
  const final = assistantMessage(completedAt.getTime(), [
    { text: "Final response", type: "text" },
  ]);
  ...
  transcript.addChild(new UserMessageComponent("Prompt", undefined, 0));
  transcript.addChild(
    new AssistantMessageComponent(final, false, undefined, undefined, 0),
  );

  const rendered = frame(transcript);
  expect(rendered.indexOf("Prompt")).toBeLessThan(rendered.indexOf("08:05"));
  expect(rendered.indexOf("08:05")).toBeLessThan(
    rendered.indexOf("Worked for"),
  );
  expect(rendered).toContain("Worked for 1m · 1 msg");
  expect(rendered.indexOf("Worked for")).toBeLessThan(
    rendered.indexOf("Final response"),
  );
  expect(rendered.indexOf("Final response")).toBeLessThan(
    rendered.indexOf("08:06"),
  );
...
it("timestamps skill-only user rows without duplicating timestamps", () => {
  ...
  state.loadHistory([
    {
      message: { content: "first", role: "user", timestamp: firstAt.getTime() },
      type: "message",
    },
    {
      message: {
...
it("compacts ten sequential tool calls while leaving the working line visible", () => {
  ...
  expect(activeFrame.indexOf("7 earlier activities")).toBeLessThan(
    activeFrame.indexOf("tool_8"),
  );
  expect(toolNames(activeFrame)).toEqual(["tool_8", "tool_9", "tool_10"]);

  const finalMessage = assistantMessage(200, [
    { text: "Final response", type: "text" },
  ]);
  ...
  expect(settledFrame.indexOf("Worked for")).toBeLessThan(
    settledFrame.indexOf("Final response"),
  );
...
it("chooses the final historical tool on the first replay frame", () => {
  ...
  expect(firstFrame.indexOf("Worked for")).toBeLessThan(
    firstFrame.indexOf("tool_3"),
  );
});
...
it("renders an interruption fallback after the worked line", () => {
  ...
  transcript.addChild(
    new AssistantMessageComponent(interrupted, false, undefined, undefined, 0),
  );
  ...
  expect(rendered.indexOf("Worked for")).toBeLessThan(
    rendered.indexOf("Operation interrupted"),
  );
  expect(transcript.render(8).every((line) => visibleWidth(line) <= 8)).toBe(
    true,
  );
...
it("keeps a pending tool error visible after the worked line", () => {
  ...
  transcript.addChild(
    new AssistantMessageComponent(failed, false, undefined, undefined, 0),
  );
  ...
  expect(rendered.indexOf("Worked for")).toBeLessThan(
    rendered.indexOf("Provider failure 2"),
  );
