...
describe("turn fold summary rendering", () => {
  ...
  it("formats normal and interrupted settled summaries", () => {
    ...
    expect(
      formatSettledSummary(summary({ aborted: true, failedTools: 1 })),
    ).toBe("▶ Worked for 1m 5s · 10 tools · 4 msgs · 1 failure · interrupted");
  ...
  it("renders themed summary lines in bold", () => {
    ...
    expect(rendered[1]).toBe(
      "<bold>▶ Worked for 1m 5s · 10 tools · 4 msgs</bold>",
    );
  });

  it("renders summaries with a leading blank row and respects zero width", () => {
    expect(
      renderStreamingSummary(summary({ running: true }), 100, undefined),
    ).toEqual(["", "▶ 7 earlier activities · 10 tools · 4 msgs"]);
    expect(
      renderSettledSummary(summary({ durationMs: 500 }), 100, undefined),
    ).toEqual(["", "▶ Worked for <1s · 10 tools · 4 msgs"]);
