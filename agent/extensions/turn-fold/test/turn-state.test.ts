...
describe("compact streaming", () => {
  it("shows only the latest three visible activity rows", () => {
    ...
    expect(
      components.map((component) => state.viewFor(component)?.display),
    ).toEqual(["streaming-summary", "original", "original", "original"]);
  });
  ...
  it("invalidates existing rows as new activity changes the compact window", () => {
    ...
    registerAssistant(
      state,
      first,
      assistantMessage(110, [{ text: "First", type: "text" }]),
    );
    const afterFirst = firstInvalidations;
    registerAssistant(
      state,
      second,
      assistantMessage(120, [{ text: "Second", type: "text" }]),
    );
    ...
    state.registerAssistantMessage(
      assistantMessage(120, [
        { text: "Second, still streaming", type: "text" },
      ]),
    );
  ...
  it("distinguishes assistant responses that share a timestamp", () => {
    ...
    const finalMessage = assistantMessage(110, [
      { text: "Done", type: "text" },
    ]);
  ...
  it("counts tool rows but not tool-call-only assistant shells", () => {
    ...
    registerAssistant(
      state,
      third,
      assistantMessage(130, [{ text: "Third", type: "text" }]),
    );
...
describe("compact settled turns", () => {
  ...
  it("keeps an interrupted assistant message visible", () => {
    ...
    registerAssistant(
      state,
      prior,
      assistantMessage(110, [{ text: "Working", type: "text" }]),
    );
  ...
  it("keeps retries in one turn and does not label a successful retry interrupted", () => {
    ...
    const success = assistantMessage(140, [
  ...
  it("keeps the latest visible message when a run settles without a final response", () => {
    ...
    registerAssistant(
      state,
      message,
      assistantMessage(110, [{ text: "Partial", type: "text" }]),
    );
...
describe("historical transcript", () => {
  it("associates user components with local-time source timestamps in order", () => {
    ...
    state.loadHistory([
      {
        message: { content: "same", role: "user", timestamp: 100 },
        type: "message",
      },
      {
      ...
      {
        message: { content: "same", role: "user", timestamp: 200 },
        type: "message",
      },
    ]);
  ...
  it("reconstructs turns and keeps each last assistant response", () => {
    ...
    const firstDone = assistantMessage(130, [
      { text: "First done", type: "text" },
    ]);
    const secondDone = assistantMessage(220, [
      { text: "Second done", type: "text" },
    ]);

    state.loadHistory([
      {
        message: { content: "first", role: "user", timestamp: 100 },
        type: "message",
      },
      ...
      {
        message: { content: "second", role: "user", timestamp: 200 },
        type: "message",
      },
  ...
  it("uses persisted completion time when restoring worked duration", () => {
    ...
    state.loadHistory([
      {
        message: { content: "prompt", role: "user", timestamp: 1_000 },
        type: "message",
      },
