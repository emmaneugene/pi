...
function summarizeAssistantContent(items: readonly unknown[]): {
  ...
  for (const item of items) {
    ...
    if (hasNonBlankContent(item, "thinking", "thinking"))
      hasVisibleContent = true;
    const toolCallId =
      stringField(item, "type") === "toolCall"
        ? stringField(item, "id")
        : undefined;
...
function assistantSnapshot(
  ...
  const { hasVisibleContent, toolCallIds } = summarizeAssistantContent(
...
function invalidateComponent(component: object): void {
  ...
  if (typeof invalidate === "function")
    Reflect.apply(invalidate, component, []);
}
...
export class TurnFoldState {
  ...
  private componentInfo = new WeakMap<
    object,
  ...
  startUserTurn(startedAt = Date.now()): string {
    const activeGroup = this.activeGroupId
      ? this.groups.get(this.activeGroupId)
      : undefined;
    if (
      activeGroup &&
      !activeGroup.startedByUser &&
      !this.groupHasActivity(activeGroup)
    ) {
  ...
  beginAssistantMessage(message: unknown): void {
    const timestamp = numberField(message, "timestamp");
    if (stringField(message, "role") !== "assistant" || timestamp === undefined)
      return;
  ...
  endAssistantMessage(message: unknown): void {
    ...
    if (numberField(message, "timestamp") !== this.activeAssistantTimestamp)
      return;
  ...
  viewFor(component: object, now = Date.now()): ComponentView | undefined {
    ...
    const display = foldDisplay({
      ...
      isStreamingSummaryAnchor:
        component === this.streamingSummaryAnchor(group),
      mode: this.mode,
  ...
  private messageAssistantKey(message: unknown): string | undefined {
    return isRecord(message)
      ? this.assistantKeyByMessage.get(message)
      : undefined;
  }
  ...
  private groupForAssistantComponent(
    ...
    if (previousComponent && previousComponent !== component)
      this.resetComponentAssociations();
    this.assistantComponentByKey.set(snapshot.key, component);
    const groupId =
      this.assistantGroupByKey.get(snapshot.key) ?? this.activeGroupId;
  ...
  private activityComponents(group: TurnGroup): [object, ComponentInfo][] {
    return [...group.components]
      .filter(
        ([candidate, info]) =>
          info.kind === "tool" ||
          group.assistants.get(candidate)?.hasVisibleContent === true,
      )
  ...
  private lastAssistant(group: TurnGroup): object | undefined {
    const candidates = [...group.assistants]
      .filter(
        ([, snapshot]) =>
          snapshot.hasVisibleContent || snapshot.hasTerminalNotice,
      )
  ...
  private componentForTool(
    group: TurnGroup,
    toolCallId: string | undefined,
  ): object | undefined {
    if (!toolCallId) return undefined;
    return [...group.tools].find(
  ...
  private summary(group: TurnGroup, now: number): FoldSummary {
    ...
    return {
      ...
      failedTools: new Set([
        ...group.failedToolCallIds,
        ...group.terminalErrorToolCallIds,
      ]).size,
  ...
  private associateComponent(
    ...
    this.componentInfo.set(component, {
      groupId: group.id,
  ...
  private indexHistoricalMessage(
    ...
    const role = stringField(message, "role");
    if (role === "assistant")
      this.indexHistoricalAssistant(group, message, completedAt);
    if (role === "toolResult")
      this.indexHistoricalToolResult(group, message, completedAt);
  }
  ...
  private invalidateGroupComponents(group: TurnGroup): void {
    for (const component of group.components.keys())
  ...
  private reloadedActiveGroups(
    ...
    const matchingGroup = visibleGroups.findIndex(
      (group) =>
        [...group.assistantKeys].some((key) =>
          activeGroup.assistantKeys.has(key),
        ) ||
  ...
  private mergeVisibleActiveGroups(
    active: TurnGroup,
    visibleGroups: readonly TurnGroup[],
  ): void {
    ...
    for (const visible of visibleGroups) {
      ...
      active.assistantKeys = new Set([
        ...active.assistantKeys,
        ...visible.assistantKeys,
      ]);
      active.failedToolCallIds = new Set([
        ...active.failedToolCallIds,
        ...visible.failedToolCallIds,
      ]);
      active.terminalErrorToolCallIds = new Set(
        visible.terminalErrorToolCallIds,
      );
      active.toolCallIds = new Set([
        ...active.toolCallIds,
        ...visible.toolCallIds,
      ]);
    }
  ...
  private reassignGroupIds(
    fromGroupIds: ReadonlySet<string>,
    toGroupId: string,
  ): void {
    for (const [key, groupId] of this.assistantGroupByKey) {
      if (fromGroupIds.has(groupId))
        this.assistantGroupByKey.set(key, toGroupId);
    }
