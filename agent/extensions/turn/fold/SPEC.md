...
This installation makes these deliberate compatibility changes:

- The `turn-diff` extension owns change summaries and reviewable patches. Turn Fold still parses upstream edit diffstats for compatibility, but its extension entrypoint suppresses them. Requirements below for summary diffstats and per-file path rows do not apply locally.
- Non-TUI sessions do not install Turn Fold render patches or transcript adapters.
- Components marked as foreign transcript components before construction, including rows in the `/subagents` viewer, retain Pi's native rendering and do not enter parent turn state.
