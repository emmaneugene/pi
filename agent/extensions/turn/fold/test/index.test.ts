...
import { processCompactionRegistry } from "../ephemeral-compactions.ts";
...
afterEach(() => {
  processCompactionRegistry().clear();
...
describe("Turn Fold headless sessions", () => {
  it("does not install patches or clear parent compactions", async () => {
    const extension = extensionHarness();
    const ctx = { ...context(), hasUI: false, mode: "print" };
    const registry = processCompactionRegistry();
    registry.clear();
    registry.remember("parent-session", {
      compactionEntryId: "parent-compaction",
      timestamp: 200,
      turnEntryIds: ["parent-user"],
      turnStartedAt: 100,
    });
    turnFold(extension.pi);
    ...
    expect(renderPatchMock.states).toEqual([]);
    expect(registry.associationsFor("parent-session").has("parent-compaction"))
      .toBe(true);
  });
});
...
describe("Turn Fold extension compaction state", () => {
  it("does not append Pi session entries for automatic compactions", async () => {
    ...
    const completedReplay = ctx.sessionManager.buildContextEntries();
    expect(completedReplay[0]).toMatchObject({ id: "compact-1", type: "compaction" });
