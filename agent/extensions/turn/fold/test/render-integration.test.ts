...
import { Container, Spacer, Text, type TUI, TuiMainScreen, type Terminal, visibleWidth } from "@earendil-works/pi-tui";
...
it("does not associate foreign assistant constructors with the active parent turn", () => {
  const state = new TurnFoldState();
  restore = installRenderPatches(state, () => undefined);
  const message = assistantMessage(100, [{ text: "Child output", type: "text" }]);
  ...
  expect(state.viewFor(ordinary)).toBeDefined();

  class ForeignAssistant extends AssistantMessageComponent {}
  markForeignTranscriptComponent(ForeignAssistant.prototype);
  const component = new ForeignAssistant(
    message,
    false,
    undefined,
    undefined,
  ...
  expect(state.viewFor(component)).toBeUndefined();
});

it("does not consume parent user associations for foreign transcript users", () => {
  const state = new TurnFoldState();
  restore = installRenderPatches(state, () => undefined);
  state.loadHistory([
    {
      message: { content: "Parent prompt", role: "user", timestamp: 100 },
      type: "message",
    },
  ]);

  class ForeignUser extends UserMessageComponent {}
  markForeignTranscriptComponent(ForeignUser.prototype);
  const foreign = new ForeignUser("Child prompt", undefined, 0);
  foreign.render(120);
  expect(state.userTimestampFor(foreign)).toBeUndefined();

  const parent = new UserMessageComponent("Parent prompt", undefined, 0);
  parent.render(120);
  expect(state.userTimestampFor(parent)).toBe(100);
});

it("does not associate foreign transcript tools with the active parent turn", () => {
  const state = new TurnFoldState();
  const ui = stoppedTui();
  restore = installRenderPatches(state, () => undefined);
  state.ensureActive(100);

  const component = markForeignTranscriptComponent(
...
it("keeps transcript order when parallel tools start in reverse order", () => {
  const state = new TurnFoldState();
  const transcript = new Container();
  const ui = stoppedTui();
  restore = installRenderPatches(state, () => undefined);
  ...
  const components = [1, 2, 3, 4].map((index) => {
    const toolCallId = `parallel-${String(index)}`;
    state.registerToolStart(toolCallId, 100 + index);
    const component = new ToolExecutionComponent(
      `tool_${String(index)}`,
      toolCallId,
  ...
  for (const component of [...components].reverse()) component.markExecutionStarted();
  for (const component of components) transcript.addChild(component);

  const activeFrame = frame(transcript);
  expect(activeFrame.indexOf("1 earlier activity")).toBeLessThan(
    activeFrame.indexOf("tool_2"),
  );
  expect(toolNames(activeFrame)).toEqual(["tool_2", "tool_3", "tool_4"]);

  state.settleActive(200);
  const settledFrame = frame(transcript);
  expect(settledFrame.indexOf("Worked for")).toBeLessThan(
    settledFrame.indexOf("tool_4"),
  );
  expect(toolNames(settledFrame)).toEqual(["tool_4"]);
});
