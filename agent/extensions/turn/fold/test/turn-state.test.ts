...
describe("historical transcript timing and reload", () => {
  ...
  it("keeps historical and active turns separate after a wider reload", () => {
    ...
    const historicalMessage = assistantMessage(20, [{ text: "Historical", type: "text" }]);
    const activeMessage = assistantMessage(110, [{ text: "Active", type: "text" }]);

    state.ensureActive(100);
    state.registerAssistantMessage(activeMessage);
    state.deferHistoryReload(() => [
      { message: { content: "Old prompt", role: "user", timestamp: 10 }, type: "message" },
      { id: "old-assistant", message: historicalMessage, type: "message" },
      { message: { content: "Current prompt", role: "user", timestamp: 100 }, type: "message" },
      { id: "active-assistant", message: activeMessage, type: "message" },
    ]);
    state.reloadHistoryForNewComponent(activeComponent);
    state.associateAssistant(historicalComponent, historicalMessage);
    state.associateAssistant(activeComponent, activeMessage);

    expect(state.viewFor(historicalComponent)?.summary).toMatchObject({
      messages: 1,
      running: false,
    });
    expect(state.viewFor(activeComponent)?.summary).toMatchObject({
      messages: 1,
      running: true,
    });
  });
