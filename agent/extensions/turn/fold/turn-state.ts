...
export class TurnFoldState {
  ...
  // Pi can retain existing transcript rows when history is rebuilt.
  private componentSequence = new WeakMap<object, number>();
  ...
  rememberTranscriptOrder(component: object): void {
    if (this.componentSequence.has(component)) return;
    this.sequence += 1;
    this.componentSequence.set(component, this.sequence);

    const existing = this.componentInfo.get(component);
    const group = existing ? this.groups.get(existing.groupId) : undefined;
    const groupInfo = group?.components.get(component);
    // The component can enter the transcript before TurnFoldState associates it.
    if (!existing || !group || !groupInfo) return;
    groupInfo.sequence = this.sequence;
    this.markGroupChanged(group);
  }
  ...
  private reloadHistory(entries: readonly unknown[]): void {
    ...
    const liveGroupId = `turn-${String(this.groupCounter + 1)}`;
    activeGroup.id = liveGroupId;
    this.pendingLiveCompactionGroups = pendingLiveCompactionGroups.map((groupId) =>
      groupId === activeGroupId ? liveGroupId : groupId,
    );
