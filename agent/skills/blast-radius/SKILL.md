...
5. Prove the one fact by applying `verify-this` to it: fresh local evidence from running the real code, pasted, not described. If you can't prove it cheaply, mark it unproven. Don't round up.
6. For a big or wide change, ask several models the same question through subagents on different providers and merge the answers. Different models catch different real bugs.
