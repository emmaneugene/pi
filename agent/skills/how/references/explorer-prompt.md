...
Start by finding the relevant code. Use `find`/`ls` to locate directories and files, `rg` to find key symbols, and `read` to understand the actual implementation. Don't guess from names. Read the code.
