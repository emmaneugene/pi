...
**Opt-in dark mode (only when the user asks for dark/adaptive).** Keep the light
`:root` above as the default and layer dark on top — never make dark the default:
...
Mermaid is the workhorse for dependency/call graphs, but everything routed through it starts to look the same. When the point is _editorial_ — weight, mass, layering — hand-build the diagram with `<div>`s and inline SVG instead. Mix these with Mermaid on the same page; don't render every diagram the same way.

Keep each ~280–340px tall so before/after sits side by side without scrolling. Use `text-xs uppercase tracking-wider` (or `--font-mono`, 11px) for in-diagram labels so they read as schematic, not UI.

### Hand-built boxes-and-arrows

Reach for this when Mermaid's auto-layout fights you, or when the "after" should feel like one thick-bordered block with greyed-out internals (a weight Mermaid won't render). Modules are bordered `<div>`s; arrows are inline SVG `<line>`/`<path>` over a `position: relative` container (see SVG Curved Connector above). Fade now-internal nodes with `opacity: 0.45`.
...
Two stacked rectangles per module: interface surface area on top, implementation below. Shallow modules show an interface rectangle nearly as tall as the implementation; deep modules show a short interface over a tall implementation. Use it to make "this interface is as wide as its implementation" visible at a glance.
...
Before: a tree of calls as nested boxes. After: the same tree collapsed into one box with the now-internal calls shown faded inside it. Pairs well with the boxes-and-arrows pattern for the "before" half.
...
Use a `data-theme` attribute only when the user explicitly wants a user-controllable light/dark toggle. **Initialize to `light`** — do not randomize the initial theme; light is the default.
...
```javascript
// Default to light. Only switch when the user toggles.
