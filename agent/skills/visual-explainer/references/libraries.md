...
<script type="module">
  ...
  // Use startOnLoad: false with the diagram-shell pattern (css-patterns.md) — the
  // engine calls mermaid.render() manually. startOnLoad: true double-renders there.
...
<script type="module">
  ...
  const isDark = false;
  mermaid.initialize({
    startOnLoad: false, // diagram-shell engine renders manually; see css-patterns.md
    theme: "base",
    look: "classic",
    themeVariables: {
...
**Aim for ≤12 nodes per Mermaid diagram.** Beyond ~12, readability drops even with zoom controls and increased fontSize; between 12 and 15 nodes, group with `subgraph` or split the diagram. At 15+ elements, use the **hybrid pattern**: a simple 5-8 node Mermaid overview showing module relationships, followed by CSS Grid cards with detailed function lists. Never cram everything into one diagram. Use `subgraph` blocks to group related nodes when under the limit:
...
> These snippets show **Mermaid syntax only**. The bare `<pre class="mermaid">` wrapper here is for brevity. In real output, place the diagram source inside the `diagram-shell` > `.diagram-source` structure from `css-patterns.md` (never ship a bare `<pre class="mermaid">`).
...
### Light by Default (Dark Opt-in)

Default to the light Mermaid palette. Mermaid initializes once and can't reactively switch themes, so hardcode the light values:

```javascript
const isDark = false; // light is the default
// Use isDark to pick light or dark values in themeVariables
```

Only when the user explicitly asks for dark or adaptive output, read the OS preference at load time instead (`const isDark = window.matchMedia("(prefers-color-scheme: dark)").matches;`) and add the matching `@media (prefers-color-scheme: dark)` CSS override. By default, do not auto-switch on `prefers-color-scheme`.
