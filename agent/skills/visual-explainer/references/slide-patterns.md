...
**Step 4 — Plan backgrounds.** For visually anchored slides — the **title slide** and any **full-bleed slide** — plan a background treatment using CSS gradients, textures, or SVG decoration that matches the deck's palette and tone. See the Proactive Imagery section below for patterns.
...
**Usage:** Instantiate after the DOM is ready. Initialize any `diagram-shell` diagrams first, then call `autoFit()` before `new SlideEngine()` so KPI/quote content is sized before intersection observers fire. (The diagram engine fits its own SVGs asynchronously and self-corrects via `ResizeObserver`, so it does not need to finish before `SlideEngine` starts.)

```html
<script type="module">
  // ...import mermaid, mermaid.initialize({ startOnLoad: false, ... })...
...
A single post-render function that handles KPI and quote overflow cases. Agents can't perfectly predict how text reflows at every viewport size, so `autoFit()` is a required safety net. Call it before SlideEngine init.

> **Do not resize `.mermaid svg` in `autoFit()`.** Mermaid diagrams are owned by the `diagram-shell` engine, which sets the SVG's width/height directly for zoom/pan. Touching those SVGs here clobbers the engine's zoom state. `autoFit()` covers KPI values and blockquotes only.
...
Two cases, one function (Mermaid is intentionally excluded — see note above):
...
Full-viewport hero. Background treatment via gradient, texture, or SVG decoration. 80–120px display type.
...
**Expand / fit.** The expand control (⛶) opens the diagram full-size in a new browser tab; double-clicking the diagram re-fits it to the viewport. The engine sets the SVG width/height directly for zoom/pan — do **not** add a `width: 100% !important` rule on `.mermaid svg`, and make sure `autoFit()` skips `.diagram-shell` SVGs (it would otherwise clobber the engine's zoom state).
...
/* diagram-shell fills the slide; the engine positions the SVG inside the viewport */
.slide--diagram .diagram-shell {
  display: flex;
  flex-direction: column;
  flex: 1;
  min-height: 0;
...
.slide--diagram .mermaid-wrap {
  ...
  flex: 1;
  min-height: 0;
  overflow: hidden; /* viewport clips; do NOT use overflow:auto with this engine */
  border-radius: 12px;
  cursor: grab;
}
.slide--diagram .mermaid-viewport {
  position: relative;
  overflow: hidden;
  width: 100%;
  height: 100%;
...
Background treatment (CSS gradient, texture, or SVG decoration) dominates the viewport. Text overlay with gradient scrim ensuring contrast. Zero slide padding.
...
Slides should reach for visuals before defaulting to text alone. If a slide could be more compelling with a background treatment, chart, or diagram, add one. Build these from CSS and SVG so the deck stays fully self-contained.

**Background treatments** — use the `.slide__bg--gradient` pattern with bold `linear-gradient` or `radial-gradient` backgrounds, layered SVG textures, or geometric decoration. Prioritize:

1. **Title slide** (always): a background that sets the deck's visual tone, matched to the preset palette (e.g. circuit/terminal aesthetic for Terminal Mono, cinematic navy+gold for Midnight Editorial, warm paper texture for Warm Signal).
2. **Full-bleed slide** (always if the deck has one): an immersive gradient/SVG background for the deck's visual anchor moment.
3. **Content slides with conceptual topics**: an SVG decoration or accent shape in the `.slide__aside` area.
...
**When to skip backgrounds:** Pure structural or data-heavy decks (code reviews, table comparisons) often read best on flat surfaces — don't force decoration where it competes with content.
...
> **Light by default.** Reach for a light preset (Warm Signal, Swiss Clean) unless the user explicitly asks for a dark deck. Each preset below is single-scheme — do **not** add `prefers-color-scheme` auto-switching by default. Use a dark preset (Midnight Editorial, Terminal Mono) only on explicit request.

### Midnight Editorial

Deep navy, serif display, warm gold accents. Cinematic, premium. Dark (opt-in — use only when the user asks for a dark deck).
...
Cream paper, bold sans, terracotta/coral accents. Confident and modern. Light (default).
...
Dark, monospace everything, green/cyan accents, faint grid. Developer-native. Dark (opt-in).
...
White, geometric sans, single bold accent, visible grid. Minimal and precise. Light (default).
